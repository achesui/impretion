...
export async function importRsaPrivateKey(pem: string): Promise<CryptoKey> {
	...
	if (!pem.trim().startsWith(pemHeader) || !pem.trim().endsWith(pemFooter)) {
		throw new Error('Formato de clave privada inválido. Se esperaba el formato PKCS#8 completo.');
	}
 ...
 */
export async function decryptField(privateKey: CryptoKey, fieldData: any): Promise<any> {
	if (fieldData?.type !== 'encrypted') {
		return fieldData;
	}
	try {
		const aesKeyBuffer = await crypto.subtle.decrypt({ name: 'RSA-OAEP' }, privateKey, base64ToArrayBuffer(fieldData.key));
		...
		return { __decryption_error__: errorMessage, __original_field__: fieldData };
	}
}
 ...
 */
export async function* readLines(textStream: ReadableStream<string>): AsyncGenerator<string> {
